...
const dropDowns = Array.from(document.querySelectorAll('#cs-navigation .cs-dropdown'));
    ...
    // JavaScript code for handling the popup form
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to check localStorage and apply class and style if needed
    function checkLocalStorage() {
        var isClosed = localStorage.getItem('closed') === 'true';
        if (isClosed) {
            CSmodal.classList.add('cs-closed');
...
// Add event listener to the "Book Today" button to show the popup form
document.addEventListener('DOMContentLoaded', function() {
    var bookTodayButton = document.querySelector('.cs-submit');

    bookTodayButton.addEventListener('click', function(event) {
        event.preventDefault(); // Prevent default form submission behavior
        var CSmodal = document.getElementById('cs-modal-1605');
        CSmodal.classList.remove('cs-close');
        CSmodal.classList.remove('cs-closed');
        CSmodal.style.zIndex = '1000'; // Set z-index to 1000 to show the modal
    });
});
